...
`purjo` (*/ˈpurjo/*) is an experimental command line tool for orchestrating [Robot Framework](https://robotframework.org/) test or task suites with the [Operaton](https://operaton.org/) BPM engine.
...
If you develop your automation package with the help of an LLM coding agent, use
the `--agents` flag to also generate an `AGENTS.md` in the project root:

```console
$ uv run --with=purjo -- pur init --agents
```

The generated `AGENTS.md` describes the package for an agent: the file layout,
the topic-to-task mapping in `pyproject.toml`, the input and output variable
conventions, failure semantics, secrets handling, the available commands, and
how to test the package. It matches the flavour you initialized, so combine it
with `--task` when you want the `*** Tasks ***` variant:

```console
$ uv run --with=purjo -- pur init --task --agents
```

`AGENTS.md` is developer-facing documentation only. It is listed in
`.wrapignore`, so it is not included in the `robot.zip` built by `pur wrap`.

The `--agents` flag is not supported together with `--python`, because the pure
Python template is still experimental.
...
*   `test_hello.robot`: An integration test that runs `hello.robot` locally, without a BPM engine.
*   `Makefile`: Convenience targets for the package. Run `make help` to list them.
*   `.wrapignore`: Specifies files to exclude when packaging the robot with `pur wrap`.
*   `AGENTS.md`: (Only with `--agents`) Guidance for LLM coding agents working on the package.
