...
the engine for tasks on a *topic*, runs the mapped Robot test, and reports the
...
2. `name` must equal the test name under `*** Test Cases ***` in `hello.robot`,
   character for character.
3. `pur serve .` reads this table to decide what to run for an incoming task.

Fields:

- `name` — the Robot test name to execute.
...
`on-fail` decides what a failing test does to the process:
...
`RobotLibrary` runs a test from another `.robot` file. Input variables are passed
